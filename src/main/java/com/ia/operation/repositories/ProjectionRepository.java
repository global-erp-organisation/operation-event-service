...
public interface ProjectionRepository extends ReactiveMongoRepository<Projection, String> {
    ...
    @Query(value = "{ 'account.user.id' : ?0, 'period.start' : { $gt : ?1 , $lt : ?2 }, operationType: ?3 }")
    Flux<Projection> findByUserAndPeriod(String userId, LocalDate start, LocalDate end, OperationType operationType);
}
