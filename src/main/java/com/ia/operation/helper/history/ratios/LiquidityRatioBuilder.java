...
public class LiquidityRatioBuilder implements RatioBuilder {
    ...
    public Mono<RateView> build(RatioParams params) {
        final Mono<List<Account>> treasuryAccounts = accountRepository.findByUser_idAndAccountType(params.getUserId(), AccountType.TREASURY).collectList();
        final Mono<List<Projection>> projections =
                projectionRepository.findByUserAndPeriod(params.getUserId(), params.getStart(), params.getEnd(), AccountType.EXPENSE).collectList();
        return treasuryAccounts.flatMap(accounts -> {
            return projections.map(pr -> {
                ...
                return RateView.builder().liquidityRatio(new BigDecimal(projection != 0 ? balance / projection : balance)).build();
            });
        });
    }
