...
public class AccountCategoryCmdHandler implements Handler {
    ...
    public Mono<ServerResponse> categoryAdd(ServerRequest request) {
        ...
        return commandComplete(bodyMono.map(body -> AccountCategoryCreationCmd.of(body).id(ObjectIdUtil.id()).build().validate(util)), gateway);
    }
    ...
    public Mono<ServerResponse> categoryUpdate(ServerRequest request) {
        ...
        return commandComplete(bodyMono.map(body -> AccountCategoryUpdateCmd.of(body).id(categoryId).build().validate(util)), gateway);
    }
